import Foundation

public struct BusStop: Equatable, Hashable, Identifiable, Sendable {
  public enum ID: Equatable, Hashable, Sendable {
    case gyeonggi(stopID: Int)
    case seoul(stopID: Int, arsID: String)

    public var stopID: Int {
      switch self {
      case let .gyeonggi(stopID), let .seoul(stopID, _):
        stopID
      }
    }

    public var storageKey: String {
      switch self {
      case let .gyeonggi(stopID):
        "gyeonggi-\(stopID)"
      case let .seoul(stopID, arsID):
        "seoul-\(stopID)-\(arsID)"
      }
    }
  }

  public let id: ID
  public let name: String
  public let alias: String?
  public let latitude: Double
  public let longitude: Double

  public init(
    id: ID,
    name: String,
    alias: String? = nil,
    latitude: Double,
    longitude: Double
  ) {
    self.id = id
    self.name = name
    let alias = alias?.trimmingCharacters(in: .whitespacesAndNewlines)
    self.alias = alias?.isEmpty == false ? alias : nil
    self.latitude = latitude
    self.longitude = longitude
  }
}

public extension BusStop {
  static let allKnown: [BusStop] = [
    .suwonStationExit7Inner,
    .suwonStationExit7Outer,
    .homaesilSsangyongApartment,
    .yeongdeungpoStation,
    .theHyundaiSeoul,
  ]

  static let suwonStationExit7Inner = BusStop(
    id: .gyeonggi(stopID: 202000106),
    name: "수원역7번출구.AK플라자",
    alias: "Inner Platform",
    latitude: 37.2674167,
    longitude: 127.0009
  )
  static let suwonStationExit7Outer = BusStop(
    id: .gyeonggi(stopID: 202000219),
    name: "수원역7번출구.AK플라자",
    alias: "Outer Platform",
    latitude: 37.2674667,
    longitude: 127.0009167
  )
  static let homaesilSsangyongApartment = BusStop(
    id: .gyeonggi(stopID: 201000096),
    name: "호매실쌍용아파트",
    alias: nil,
    latitude: 37.2678,
    longitude: 126.9513167
  )
  static let yeongdeungpoStation = BusStop(
    id: .seoul(stopID: 118000005, arsID: "19005"),
    name: "영등포역",
    alias: nil,
    latitude: 37.5158657465,
    longitude: 126.90509208
  )
  static let theHyundaiSeoul = BusStop(
    id: .seoul(stopID: 118000197, arsID: "19282"),
    name: "더현대서울",
    alias: nil,
    latitude: 37.5250045778,
    longitude: 126.9281836353
  )
}
